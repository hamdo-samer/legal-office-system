...
export default function LoginPage() {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    try {
      ...
      alert(error instanceof Error ? error.message : t('loginError'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleGoogleLogin = async () => {
    try {
      // يمكن إضافة تكامل مع Google OAuth هنا لاحقاً
      alert('🚧 ' + t('loginWithGoogle') + ' سيتم إضافته قريباً!\n\nيرجى استخدام تسجيل الدخول العادي حالياً.');
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit(onSubmit)}>
          <div className="space-y-4">
            ...
            <div>
              ...
              {errors.password && (
                <p className="mt-1 text-sm text-red-600">{t('passwordMinLength')}</p>
              )}
            </div>
          </div>
          ...
          <div className="mt-6">
            ...
            <div className="mt-6">
              <button
                ...
                className="w-full inline-flex justify-center items-center py-3 px-4 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-200"
              >
